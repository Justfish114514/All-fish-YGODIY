...
function cm.ffilter(c)
	return c:IsFusionSetCard(0x1400) or c:IsRace(RACE_ZOMBIE) or c:IsRace(RACE_MACHINE)
end
function cm.splimit(e,se,sp,st)
	return e:GetHandler():GetLocation()~=LOCATION_EXTRA
end
function cm.spfilter(c)
	return (c:IsFusionSetCard(0x1400) or c:IsRace(RACE_ZOMBIE) or c:IsRace(RACE_MACHINE)) and (c:IsType(TYPE_MONSTER) or bit.band(c:GetOriginalType(),TYPE_MONSTER)==TYPE_MONSTER) and c:IsCanBeFusionMaterial() and c:IsAbleToDeckOrExtraAsCost()
end
...
function cm.pcfilter1(c,e,tp,ft)
	local lv=c:GetLevel()
	return lv>0 and c:IsSetCard(0x1400) and (c:IsLocation(LOCATION_GRAVE) or (c:IsLocation(LOCATION_EXTRA) and c:IsFaceup())) and not c:IsForbidden() and  Duel.IsExistingMatchingCard(cm.pcfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,nil,e,tp,lv) and c:IsAbleToDeckOrExtraAsCost()
end
function cm.pcfilter(c,e,tp,lv)
	return c:IsSetCard(0x1400) and c:IsType(TYPE_PENDULUM) and c:GetLevel()==lv 
end
...
function cm.filter(c,e,tp)
	return (c:IsSetCard(0x1400) and c:IsLocation(LOCATION_GRAVE) and c:IsAbleToDeckOrExtraAsCost()) or (c:IsSetCard(0x1400) and c:IsFaceup() and c:IsLocation(LOCATION_EXTRA) and c:IsAbleToDeckAsCost())
end
