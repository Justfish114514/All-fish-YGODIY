...
function cm.lfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x1404) and c:IsControler(tp) and c:IsAbleToDeck() and c:IsOnField()
end
...
function cm.cfilter(c,seq2)
	...
	return c:IsFaceup() and c:IsSetCard(0x1404) and c:IsAttackPos() and seq1==4-seq2
end
