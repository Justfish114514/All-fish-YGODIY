...
function cm.distg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return (Duel.GetLocationCount(tp,LOCATION_SZONE)>0 or eg:GetFirst():IsLocation(LOCATION_SZONE)) and (eg:GetFirst():IsControler(tp) or (eg:GetFirst():IsAbleToChangeControler() and not c:IsLocation(LOCATION_SZONE))) and c:GetFlagEffect(14000205)~=0 and re:GetHandler()~=c and not c:IsStatus(STATUS_BATTLE_DESTROYED) end
end
